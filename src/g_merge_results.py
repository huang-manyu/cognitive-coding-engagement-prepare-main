...
def load_image_descriptions(task_dir: Path):
    """读取 image_understanding 下所有 JSON，根据 frames/config.json 的 interval 计算时间戳"""
    config_path = task_dir / "frames" / "config.json"
    img_dir = task_dir / "image_understanding"

    if not config_path.exists() or not img_dir.exists():
        print("未找到图像理解结果，跳过图像合并")
        return []

    with open(config_path, encoding="utf-8") as f:
        interval = json.load(f)["interval"]

    images = []
    for p in sorted(img_dir.glob("image_*.json"), key=lambda x: int(re.search(r"(\d+)", x.stem).group(1))):
    ...
    print(f"图像描述: {len(images)} 张，间隔 {interval}s")
    return images
...
def merge_results(task_id: str):
    ...
    # 读取图像描述并加入时间线
    images = load_image_descriptions(task_dir)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("用法: uv run src/g_merge_results.py <task_id>")
        sys.exit(1)
